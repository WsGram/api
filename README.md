# WsGram Telegram Bot API
The official documentation is [here](https://wsgram-team.github.io/wsgram-telegram-bot-api/).

WsGram Telegram Bot API is an actively enhanced fork of the original Bot API, featuring experimental user support, proxies, unlimited files size, and more.

WsGram Telegram Bot API is 100% compatible with the official version.
...
If you've got any questions about our fork, instead, join [@WsGramChat](https://t.me/WsGramChat) in Telegram, or see the news at [@WsGram](https://t.me/WsGram).

Please note that only WsGram-specific issues are suitable for this repository.

## Table of Contents
- [WsGram Bot API features](#wsgram-features)
...
<a name="wsgram-features"></a>
## WsGram Bot API features
...
```bash
docker run -p 8081:8081 --env TELEGRAM_API_ID=API_ID --env TELEGRAM_API_HASH=API_HASH wsgram/wsgrambotapi
```

The simplest way to build `Telegram Bot API server` is to use our [Telegram Bot API server build instructions generator](https://wsgram-team.github.io/wsgram-telegram-bot-api/build.html).
...
```bash
git clone --recursive https://github.com/wsgram-team/wsgram-telegram-bot-api
cd wsgram-telegram-bot-api
mkdir build
cd build
